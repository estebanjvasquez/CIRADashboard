import { buildInvalidJsonDiagnostics, diagnosticsToCsv } from '../../../src/etl/metrics';
...
export const onRequestGet: PagesFunction<Env> = async ({ env, request }) => {
  ...
  const diagnostics = buildInvalidJsonDiagnostics(rows, {
  ...
  return new Response(diagnosticsToCsv(diagnostics.rows), {
    headers: {
      'Content-Disposition': 'attachment; filename="diagnostico-json-invalido.csv"',
